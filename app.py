...
with gallery_tab:
    ...
    # 1) Pobierz stockowe i userowe pliki
...
with reset_tab:
    if st.button("🔄 Restart MyGallery App"):
            # 1. Usuń pliki sesyjne
            for fname in os.listdir(save_dir):
                os.remove(os.path.join(save_dir, fname))

            # 2. Usuń punkty sesyjne z Qdranta
            qdrant_client = get_qdrant_client()
            ...
            # 3. Wyczyść cały session_state
            st.session_state.clear()

            # 4. Nadpisz TYLKO klucze, które chcesz zachować/resetować
